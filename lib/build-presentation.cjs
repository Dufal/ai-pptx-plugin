...
const SLIDE_TYPES = ['title', 'content', 'data', 'features', 'closing'];
 ...
 */
async function generateFallbackBackgrounds(outputDir, styleDescription) {
  const gradients = {
  ...
  for (const type of SLIDE_TYPES) {
    ...
    const outPath = path.join(outputDir, `bg-${type}.png`);
    await sharp(Buffer.from(svg)).png().toFile(outPath);
    results[type] = outPath;
  }
 ...
 */
async function generateWhiskBackgrounds(outputDir, styleDescription, refPaths) {
  ...
  // Phase 1b: Generate title background first (as style anchor)
  const titlePrompt = buildBgPrompt(styleDescription, 'title');
  let titleResult;

  if (userRefs.length > 0) {
    titleResult = await whisk.generateWithReference(titlePrompt, '16:9', token, userRefs);
  } else {
    titleResult = await whisk.generateImage(titlePrompt, '16:9', token);
  }

  if (!titleResult.success) {
    console.error('Title background generation failed:', titleResult.error);
    return null;
  }

  const titlePath = path.join(outputDir, 'bg-title.png');
  whisk.saveBase64Image(titleResult.images[0], titlePath);
  results.title = titlePath;

  // Phase 1c: Upload title as additional style reference for consistency
  const titleBase64 = titleResult.images[0];
  const titleRef = await whisk.uploadAndAnalyze(titleBase64, 'MEDIA_CATEGORY_STYLE', token);
  const consistencyRefs = [...userRefs];
  if (titleRef.success) {
    consistencyRefs.push({
      category: 'MEDIA_CATEGORY_STYLE',
      mediaId: titleRef.mediaId,
      caption: titleRef.caption,
    });
  }

  // Phase 1d: Generate remaining backgrounds with consistency refs
  const remaining = SLIDE_TYPES.filter(t => t !== 'title');
  for (const type of remaining) {
    const prompt = buildBgPrompt(styleDescription, type);
    let result;
    ...
    if (!result.success) {
      console.error(`Background for "${type}" failed: ${result.error}, using fallback`);
      const fallbacks = await generateFallbackBackgrounds(outputDir, styleDescription);
      results[type] = fallbacks[type];
      continue;
    }
 ...
 * @param {object} config.slides - Slide content data
...
async function buildPresentation(config) {
  ...
  let backgrounds = await generateWhiskBackgrounds(imagesDir, style, refs);

  if (!backgrounds) {
    console.log('Whisk unavailable, using gradient fallbacks...');
    backgrounds = await generateFallbackBackgrounds(imagesDir, style);
  }

  console.log('Backgrounds ready:', Object.keys(backgrounds).join(', '));
  ...
  const slideConfigs = [
    { type: 'title', fn: templates.titleSlide, data: slides.title },
    { type: 'content', fn: templates.contentSlide, data: slides.content },
    { type: 'data', fn: templates.dataSlide, data: slides.data },
    { type: 'features', fn: templates.featuresSlide, data: slides.features },
    { type: 'closing', fn: templates.closingSlide, data: slides.closing },
  ];

  for (let i = 0; i < slideConfigs.length; i++) {
    const { type, fn, data } = slideConfigs[i];
    if (!data) {
      console.log(`  Skipping slide ${i}: ${type} (no data)`);
      continue;
    }
    const slideData = { ...data, bgImage: backgrounds[type] };
    const html = fn(slideData);
    const filePath = writeSlideHtml(outputDir, type, html, i);
...
module.exports = { buildPresentation, generateFallbackBackgrounds, renderIconPng };
